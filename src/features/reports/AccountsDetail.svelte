<script lang="ts">
	...
	import { axisMonthLabel, pointsInRange } from '$features/reports/net-worth';
...
<ReportBody
	loading={(!accounts.data || !history.data) && !accounts.error && !history.error}
	stale={accounts.stale || history.stale}
>
					...
					<ul class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
						{#each charted as d, i (d.key)}
							<li class="flex min-w-0 items-center gap-1.5">
								<span
									class="size-2.5 shrink-0 rounded-full {segmentClass(i + 1)}"
									aria-hidden="true"
								></span>
