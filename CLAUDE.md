...
- `src/features/backup/cloud/`: automatic backups to the user's cloud storage, main thread only. A `CloudProvider` (`provider.ts`; Google Drive in `google-drive.ts`, plain `fetch` to its REST API) signs in once with OAuth + PKCE in a popup (`oauth.ts`; it lands on `/oauth/callback`, which answers over a BroadcastChannel) and keeps its tokens in IndexedDB `moneta-cloud` (`tokens.ts`). `cloud-backup.ts` uploads through the usual `backUp()` and rotates (`retention.ts`: one file per device and day, the newest and five before it); `auto-backup.ts` decides when; `cloud.svelte.ts` holds the state for the screens and runs it in `AppShell`. Tests use the fake Drive in `fake-drive.ts`.
...
- No COOP/COEP headers or server code: the OPFS SAH-pool VFS doesn't need them, and the build must work on any static host. The one exception is the optional, stateless token function in `netlify/` (Google requires a client secret for refresh tokens, and it must not ship in the app): it stores nothing, and without `VITE_GOOGLE_CLIENT_ID` in the build the cloud backup option is hidden.
- Backups to the cloud are always encrypted: `cloud-backup.ts` refuses bytes that aren't. A provider's `connect()` opens its popup before anything is awaited, so call it straight from a click.
- The Content-Security-Policy lives in `svelte.config.js` (`kit.csp`, hash mode, emitted as a `<meta>` tag). New external origins, inline scripts or `{@html}` must fit it; `e2e/csp.e2e.ts` fails on any violation. `connect-src` allows Google's API origins for Drive backups; a new provider adds its own.
